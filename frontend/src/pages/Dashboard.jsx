    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        >
          ...
          <p className="text-sm text-gray-500 mt-1">Add or edit employee records</p>
        </Link>
        <Link
          to="/attendance"
          className="bg-white p-6 rounded-lg shadow hover:shadow-md transition-shadow"
        >
          ...
          <p className="text-sm text-gray-500 mt-1">Check attendance records</p>
        </Link>
        <Link
          to="/devices"
          className="bg-white p-6 rounded-lg shadow hover:shadow-md transition-shadow"
        >
          ...
          <p className="text-sm text-gray-500 mt-1">Configure biometric devices</p>
        </Link>
      </div>
              ...
              <table className="min-w-full">
                ...
                <tbody>
                  {todayAttendance.slice(0, 10).map((record, index) => (
                    <tr key={record.id || index} className="border-b last:border-0">
                      <td className="py-3 px-4">
                        <div className="font-medium">{record.employee_name || record.employee?.full_name || "N/A"}</div>
                        <div className="text-sm text-gray-500">{record.employee_code || record.employee?.employee_code || ""}</div>
                      </td>
